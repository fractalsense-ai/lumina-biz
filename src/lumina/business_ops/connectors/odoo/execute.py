"""Fixture-only execution seam for the Odoo Slice 34 connector."""
...
def execute_with_fixtures(
    ...
    *,
    ...
    try:
        mapped = map_operation_to_odoo(request)
    ...
    if status == "failed":
            ...
            for error in source_errors:
                ...
                normalized_errors.append(
                    normalize_odoo_error(
                        ...
                        message=str(error.get("message") or "Odoo fixture execution failed"),
                        action_class=action_class,
                        capability_namespace=capability_namespace,
                        provider_error_code=(
        ...
        if not normalized_errors:
            normalized_errors.append(
                normalize_odoo_error(
                    status_code=None,
                    message="Odoo fixture execution failed",
                    action_class=action_class,
                    capability_namespace=capability_namespace,
                )
            )
