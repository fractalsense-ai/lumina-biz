...
| **Framework core** | **Core Engine** | `src/lumina/` | Zero domain-specific names — reads only `task_ready_for_execution` and `task_status` |
...
|---|---|---|
| `runtime_adapters.py` | **Primary controller** | Synthesis layer — runs Phase A (NLP pre-processing) and Phase B (signal synthesis after tools); emits engine contract fields `task_ready_for_execution` and `task_status` |
