"""Business Ops POC for the signal-decomposition framework (Phase H.6).
...
domain (business-ops) with totally different actor signals — soil pH,
soil moisture, motor vibration — declared in business-ops's
`domain-physics.json` `signals` block with per-signal `record_path`
extractors and per-signal band overrides.

This test exercises the framework as a black box from the business-ops
domain's perspective:

1.  The business-ops pack's `to_signal_samples()` adapter converts
    ...
    keys are the business-ops-domain signal names (no SVA bias).
2.  Synthetic field records carrying nested `sensors.soil.pH` readings
    are routed through the daemon task using the business-ops physics
    ...
    `signal=soil_pH, band=dc_drift, direction=-1` — proving that:
       ...
       b. The framework writes per-signal advisories whose `signal`
          field matches the business-ops-declared name verbatim.
       c. Per-signal `message_overrides` are honored (the rendered
          advisory message contains the override's `{label}` template
          substitution).
3.  Loading the business-ops domain physics from disk validates the
...
def _load_environmental_sensors():
    """Dynamically load the business-ops group library (its parent directory
    contains dashes so it isn't importable as a regular Python package)."""
    import importlib.util
    import sys
    mod_name = "field_ops_environmental_sensors_under_test"
...
def _field_ops_physics() -> dict[str, Any]:
    return json.loads(FIELD_OPS_PHYSICS_PATH.read_text(encoding="utf-8"))
...
class TestFieldOpsPhysicsDeclaresSignals:

    def test_physics_loads_from_disk(self):
        physics = _field_ops_physics()
        assert physics["id"].startswith("domain/bizops/")

    def test_signals_block_present_with_expected_signals(self):
        physics = _field_ops_physics()
        assert "signals" in physics, "business-ops physics should declare a signals block"
...
# ─────────────────────────────────────────────────────────────
# 2. Business Ops's to_signal_samples adapter feeds the framework
# ─────────────────────────────────────────────────────────────


class TestFieldOpsSignalSampleAdapter:
    ...
    def test_framework_baseline_keyed_by_field_ops_signal_name(self):
        """Framework writes per-signal entries under the business-ops name."""
        sensors_mod = _load_environmental_sensors()
        SensorReading = sensors_mod.SensorReading
        to_signal_samples = sensors_mod.to_signal_samples
        readings = [
        ...
        # No SVA-named entries leaked into the business-ops baseline
...
class TestFieldOpsChronicDriftEndToEnd:
    ...
    def test_chronic_soil_ph_drift_emits_proposal_with_signal_name(self):
        ...
        # by the business-ops-declared name, NOT that any one specific
    ...
    def test_advisory_persisted_uses_per_signal_message_override(self):
        """The business-ops physics declares a circaseptan,* override for
...
class TestFieldOpsAdvisoryConformsToSchema:
