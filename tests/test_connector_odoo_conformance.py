...
@pytest.mark.unit
def test_odoo_manifest_conforms_to_canonical_schema() -> None:
...
def test_fixture_execution_result_conforms_to_operation_result_schema() -> None:
    runner = DeterministicFixtureRunner(
        [
            FixtureScenario(
                ...
                result_payload={"status": "succeeded", "data": {"records": [{"name": "TICKET-1"}]}} ,
            )
        ]
    )
    ...
    result = execute_with_fixtures(
        {
            "request_id": "req-odoo-1001",
            "action_class": "query",
            "capability_namespace": "service/work-order",
            "payload": {"filters": {"name": ["like", "TICKET-%"]}},
            "actor_scope": {
...
def test_unsupported_mapping_returns_failed_result_with_schema_valid_error() -> None:
    ...
    result = execute_with_fixtures(
        {
            "request_id": "req-odoo-unsupported",
